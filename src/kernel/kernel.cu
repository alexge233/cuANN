...
__global__ void sigmoid_activation( float * input)
{
    ...
    // Update value
    input[x]  = sigmoid_function( input[x] );  
}

__global__ void sigmoid_prime( float * sum_ji, float * output )
{
    // Iterate Vector `Σ[ji]`
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    // get sigmoid output
    float sigmoid = sigmoid_function( sum_ji[x] );
    // Sigmoid Prime: σ(x) * (1 - σ(x))
    output[x] = __fmul_rz( sigmoid, (1.f - sigmoid) );
}
                             ...
                             )
{
    ...
    float sigmoid = sigmoid_function( sum[x+index] );
    // Sigmoid Prime: σ(x) * (1 - σ(x))
    float primed = __fmul_rz( sigmoid, (1.f - sigmoid) );
    // -E * σ'(Σ(O[i])
